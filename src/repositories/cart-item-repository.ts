...
export async function findById(id:number): Promise<null|CartItem> {
  const findItem :QueryResult<CartItemRow> = await db.query(`SELECT * FROM cart_items WHERE id = $1`,[id]);
  return !findItem.rows[0] ? null : camelCaseKeys(findItem.rows[0]) as CartItem;
}

export async function updatedQuantity(id:number,quantity:number): Promise<null|CartItem> {
  const sentence:QueryResult<CartItemRow> = 
  await db.query(`UPDATE cart_items SET quantity = $2, update_at = NOW() WHERE id = $1 RETURNING *`,
    [ 
      id,
      quantity
    ]);
  return !sentence.rows[0] ? null : camelCaseKeys(sentence.rows[0]) as CartItem;
